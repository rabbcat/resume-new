...
var _bbtInner = new Object(
		{
			...
			recLand : function(btIsReg){
				...
				if(params.indexOf("?")==0){
					for ( var p in dspParamsArray) {
						if(params.indexOf(dspParamsArray[p]) > 0 && _bbtcookie.cookie("BTKR_TNID")!=="" && _bbtcookie.cookie("BTKR_TNID")!=="undefined"){
							$.ajax({
								url:'https://btracker.cjddealer.com.cn/btracker/images/land.png',
								dataType: "jsonp",
								jsonp: "callback",
								cache:false,
								async : true,
				...
				if('reg'==btIsReg){
					$.ajax({
						url:'https://btracker.cjddealer.com.cn/btracker/images/reg.png',
						dataType: "jsonp",
						jsonp: "callback",
						cache:false,
						async : true,
			...
			trackPage : function() {
				...
				(function() {
					setTimeout(
							function() {
								$.ajax({  
                   						url:'https://btracker.cjddealer.com.cn/btracker/images/bbtrack.png',
										dataType: "jsonp",
										jsonp: "callback",
										cache:false,
		                                async:true,  
